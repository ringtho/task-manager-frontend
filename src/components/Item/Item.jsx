...
const Item = (props) => {
  const {
    title, id, subtasks, description, completed, setIsEditItemActive, setItemId, setIsDeleteItemActive, setList, list
  } = props
  const [isComplete, setIsComplete] = useState(completed)
  const [isActive, setIsActive] = useState(false)

  const handleEditClick = () => {
    setItemId({ id, title, subtasks, description, completed })
      ...
      >
        ...
        <div className="item__text-wrapper">
          <p className="item__text">
            {title} <span className="item__quantity">({subtasks.length} subtasks)</span>
          </p>
